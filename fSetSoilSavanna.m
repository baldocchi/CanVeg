function [soil] = fSetSoilSavanna(met,prm, soil)
...
% Site Summary for Soil Texture: Median and Stdev (σ
% )
% Site	Clay [%]	Silt [%]	Sand [%]	Root Mass [g]	Bulk Den [kg m-3]	Clay σ
% Silt σ
% Sand σ
% Root Mass σ
% Bulk Den σ
% US-TON	23.65	53.4	19.25	0.0211	1.351728	14.5455	11.45731	4.646863	0.164965	0.1370892
% 
...
%        Soil water content 
 ...
 %       // Water content of litter. Values ranged between 0.02 and 0.126
  ...
  %   airborne fraction = porosity - volumetric water content
        ...
        soil.bulkdensity=1.35;     % g cm-3   Data from Indiana
        ...
        soil.clay_fraction = .23;      %//  Clay fraction   
        soil.sand_fraction = 0.19;     % sand fraction
        soil.silt_fraction = 0.53;
		soil.peat_fraction = 0.0;    %//  SOM = a C; C = 4.7%, a = 1.72  Kuno Kasak, 2019 Bouldin Alfalfa
       
        
		soil.mineral_fraction= 1-soil.pore_fraction -soil.silt_fraction ; % // from bulk density asssuming density of solids is 2.65
       ...
       % soil.vol(1)=0;
        soil.vol = aream2 * soil.dz;   % soil volume
         ...
         soil.bulk_density=ones(prm.nn,soil.n_soil) .* soil.bulkdensity;   % /soil  bulk density for the alfalfa, g cm-3
