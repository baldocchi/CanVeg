 function [ps]=fLeafPsMatrix(Iphoton,cca, Tlk, rb_co2, P_kPa, eair_Pa,prm)
 

%         4-25-2019, Leaf_photosynthesis model   
...
%          ps.rd = zeros(1,prm.jktot);
            ...
            gs_leaf_mole=zeros(prm.nn,prm.jktot);
...
% */
    ...
    %%%%%make sure I am querying the wj and wc correctly  
    % may need logical loop
...
%                 // real roots 
                        ...
                        root2 = -2.0 .* sqrt(Q) .* cos((ang_L + pi*2) / 3.0) - Pcube / 3.0;
                        root3 = -2.0 .* sqrt(Q) .* cos((ang_L - pi*2) / 3.0) - Pcube / 3.0;

%                 // rank roots #1,#2 and #3 according to the minimum, intermediate and maximum
%                 // value

minroot=zeros(prm.nn,prm.jktot);
midroot=zeros(prm.nn,prm.jktot);
maxroot=zeros(prm.nn,prm.jktot);
aphoto=zeros(prm.nn,prm.jktot);
...
% */
        ...
        gs_m_s(j,i) = gs_leaf_mole(j,i) * Tlk(j,i) * 101.3*0.022642 /(P_kPa(i)*273.15) ;
...
%         negative, which is nonsense
         ...
         ps_1(j,i) = cca(i) * gb_mole(j,i) * gs_co2(j,i);
...
% */
        ...
        cs = cca(i) - aphoto(j,i) / gb_mole(j,i);
        ci = cs - aphoto(j,i) / gs_co2(j,i);
        ...
        %  may need to reconsider prof from prm.nn and prm.jktot
